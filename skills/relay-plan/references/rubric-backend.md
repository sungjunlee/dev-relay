...
- **fix_hint**:
  - low_to_mid: Add timeout (default 5s) to every external HTTP/DB call; gate retries behind an idempotency check
  - mid_to_high: Add exponential backoff with jitter to retries; wrap downstream calls in a circuit breaker (open after 3 consecutive failures); return structured error with caller-actionable message
...
- **fix_hint**:
  - low_to_mid: Add UNIQUE/NOT NULL/FK constraints to critical fields; wrap multi-step mutations in a transaction
  - mid_to_high: Add idempotency key header to all mutation endpoints; run EXPLAIN on new queries with expected production row counts
