...
function sandboxInvocation({ role, command, args = [], readRoots = [], writeRoots = [], readFiles = [], writeFiles = [], denyWriteFiles = [],
  ...
  // Go's darwin `crypto/x509` (every `gh` TLS handshake) evaluates the chain through Security.framework,
  // which reaches the per-user trust daemon. Denying that one lookup surfaces as `OSStatus -26276`
  // (errSecInternalComponent) rather than a trust verdict. The root `com.apple.trustd` is measurably
  // neither used nor sufficient here, so only the agent name is granted, and only with network access.
