...
- Delegating implementation to an executor (`codex`, `claude`, `opencode`, `pi`, `antigravity`) via worktree isolation
...
| `--executor, -e` | Executor: `codex` (default), `claude`, `opencode`, `pi`, `antigravity` |
| `--model, -m` | Model override; for `opencode`, falls back to bundled `references/executor-models.json`, then optional `~/.relay/executors.json` override; Pi and Antigravity should pass explicit routes such as `openai/gpt-5` or `google/antigravity-cli` |
...
| `--timeout` | Timeout in seconds (default: codex 2400; claude/opencode/pi/antigravity 1800). Antigravity maps this to `agy --print-timeout <seconds>s`. |
