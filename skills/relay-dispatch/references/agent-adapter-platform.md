...
```bash
node invoke-reviewer-<name>.js --repo <repoPath> --prompt-file <promptPath> --json [--model <route>]
```
...
3. Add reviewer scripts only for phases the descriptor marks supported. Keep OpenCode-style advisory-only adapters fail-closed for primary review until they produce trusted verdict JSON.
