...
> Variant of `prompt-template.md` for an executor whose dispatch toolset has no command-execution tool. Resolve that first with `node "${RELAY_SKILL_ROOT:-skills}/relay-config/scripts/relay-config.js" check --executor <name> --phase dispatch --json`: when `capability.commandExecution` is `false` — today Pi — emit this template instead of the base one.
>
