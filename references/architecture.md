...
The two slices are independent and never merged. A run whose assigned reviewer is `codex` but whose round-1 `review_apply` event carries `reviewer: "claude"` shows up under `by_role.reviewer.codex` *and* `by_acting_reviewer.claude`. Runs with zero reviewer-tagged `review_apply` events do not appear in the acting-reviewer slice at all — use `--by-role` to see reviewer backlog (runs assigned but not yet started).

System-emitted `review_apply` events that carry no `reviewer` field (for example, the `max_rounds_exceeded` escalation path in `review-runner.js`) are skipped rather than bucketed — no reviewer actually performed those rounds. Events that carry an explicit but empty or corrupt `reviewer` value do land in an `"unknown"` bucket so data-integrity issues surface in the report instead of being silently dropped.

Each acting-reviewer bucket includes a `review_rounds_performed` counter — the number of `review_apply` events that reviewer actually executed within the scoped runs. This is the round-exclusive metric to use for Codex-vs-Claude execution comparisons. The other rollup metrics (`metrics.median_rounds_to_ready`, `metrics.stale_open_runs_72h`, `totals.manifests`) are run-level: for a run that mixed reviewers across rounds, those values reflect the whole run in every bucket it contributed to, not just the rounds that specific reviewer performed.
