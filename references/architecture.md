...
| Route plan | Dispatch writes `route-plan.json` in the run directory with effective dispatch/review/advisory/sidecar routes, source traces, and policy decisions. The manifest stores only a compact `routes.summary` plus `routes.plan_path` so operators can audit routing without bloating the lifecycle contract. |
