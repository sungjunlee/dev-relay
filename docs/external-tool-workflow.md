...
This doc captures where each tool naturally fits relative to relay's lifecycle (plan → dispatch → review → merge → retro). Companion to [`workflow-lanes.md`](./workflow-lanes.md): that one decides *which lane to use*; this one decides *which tools to invoke inside that lane*.
...
**Boundary**: do **not** treat CE as a fourth orchestration step (plan → dispatch → review → CE-review → merge). That double-counts the review surface, doubles operator cost, and makes the merge gate ambiguous about whose verdict counts. The relay reviewer is the merge gate; CE is supplementary.
