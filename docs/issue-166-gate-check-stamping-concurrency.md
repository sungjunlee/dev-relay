...
| `relay-resolver.js:45-50,93-113,252-253` | `git.pr_number` reads | read-only diagnostics/selectors | **UNCHANGED — read-only**. These sites render or compare stored PR values; they do not mutate manifests or emit `pr_number_stamped`. |
