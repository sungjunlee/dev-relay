...
- Rule 3, end-to-end regression: the new regression spawns real `gate-check.js` child processes that share `RELAY_HOME` and the same fake PR payload, then asserts on committed `events.jsonl` contents. The same test fails against a temporary `HEAD` copy of pre-fix `gate-check.js`, where it reproduced 3 duplicate `pr_number_stamped` rows.
...
| `finalize-run.js:396-414` | `git.head_sha` + `merge_finalize` | state-transition-gated finalize write | **UNCHANGED — state-machine-gated**. `updateManifestState()` flows through `validateTransition()` at `relay-manifest.js:367-377`, so only the first `READY_TO_MERGE -> MERGED` write succeeds. |
...
| `review-runner.js:1189-1196,1305-1312,1394-1400` | `review_apply` | per-round transition audit | **UNCHANGED — state-machine-gated**. Round application writes are coupled to manifest transitions and round advancement, not first-resolution stamping. |
...
| `dispatch.js:497-502` | `environment_drift` | resume-path audit | **UNCHANGED — state-machine-gated**. Emitted during the same-run resume flow that already requires a validated active manifest and a legal `changes_requested -> dispatched` progression. |
| `dispatch.js:542-548` | `rubric_grandfathered` | grandfather-only resume audit | **UNCHANGED — state-machine-gated**. Same resume gating as `environment_drift`; not a first-resolution field stamp. |
| `dispatch.js:817-823,968-975` | `dispatch_start` / `dispatch_result` | dispatch lifecycle | **UNCHANGED — state-machine-gated**. Initial dispatch owns run creation; resume dispatches already go through `updateManifestState()` and `validateTransition()`. |
| `close-run.js:98-115` | `close` + `cleanup_result` | terminal transition audit | **UNCHANGED — state-machine-gated**. `CLOSED` is terminal, so concurrent double-close attempts fail via `validateTransition()`. |
...
The sole first-resolution stamping site claim still holds on this head: `gate-check.js` is the only path that reads `git.pr_number === null`, discovers the PR from external state, writes it back, and emits a corresponding audit row. Everything else above is either caller-provided, single-writer initialization, per-attempt audit, or state-machine-gated.
...
- `skills/relay-dispatch/scripts/relay-resolver.js` — `#156` / `#174` / `#177` containment fixes are complete; `#166` is orthogonal.
