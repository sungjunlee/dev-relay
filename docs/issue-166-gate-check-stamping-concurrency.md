...
| `dispatch.js:817-823,968-975` | `dispatch_start` / `dispatch_result` | dispatch lifecycle per invocation | **UNCHANGED — per-dispatch invocation**. Initial dispatch owns run creation (`createRunId()` yields a unique-per-ms timestamp, so concurrent initial dispatches land in different run directories). `dispatch_result` emits on the same per-invocation lifecycle as its matching `dispatch_start`; resume dispatches share the separate concurrency concern noted on the `environment_drift` row above. |
