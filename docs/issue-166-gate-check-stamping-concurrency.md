...
- Rule 4, state-machine-axis applicability with helper reuse: the relay state machine is intentionally untouched because `#166` is not a bad transition bug. The defect sits below `validateTransition()` in a first-resolution write path that already converged at the manifest layer. The `#177` `isNonTerminalState` whitelist predicate is now reused inside `stampPrNumberUnderLock()` instead of reinvented locally — consistent with the meta-rule 6 "reuse the validator, don't reinvent" discipline established by `#176` / PR #183's `safeFormatRunId` export. The `relay-resolver.js` edit is strictly an export addition; the function body and existing call sites are unchanged.
...
| `relay-resolver.js:485` (`isNonTerminalState` export) | exported helper | shared state-axis whitelist predicate | **EXPORTED** — added to `module.exports` so `gate-check.js` reuses the `#177` whitelist (fail-closed on tampered state). Body unchanged; same scoped-export precedent as `#176` / PR #183 added `safeFormatRunId`. |
...
- `skills/relay-dispatch/scripts/relay-resolver.js` — `#156` / `#174` / `#177` containment fixes are complete; `#166` touches this file EXCLUSIVELY to add `isNonTerminalState` to `module.exports` so `gate-check.js` can reuse the `#177` whitelist helper (fail-closed on tampered/missing state). Same scoped-export precedent as `#176` / PR #183 added `safeFormatRunId`. The function body itself and its internal call sites are unchanged.
