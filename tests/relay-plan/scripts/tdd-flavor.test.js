...
const FINALIZE_RUN_SCRIPT = path.join(__dirname, "..", "..", "..", "skills", "relay-merge", "scripts", "finalize-run.js");
...
test("reference docs document the exact two-cell TDD state matrix and avoid top-level tdd_mode", () => {
  ...
  const reviewSchema = fs.readFileSync(path.join(__dirname, "..", "..", "..", "skills", "relay-review", "scripts", "review-schema.js"), "utf-8");
  const matrix = [
