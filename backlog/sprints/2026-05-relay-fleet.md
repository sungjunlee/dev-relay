...
- **Sprint-fleet wiring is opportunistic**, not enforced. design.md resolved-question #3: "one fleet per sprint batch is the natural mapping; wire opportunistically." Sprint files do not auto-record fleet-id; if traceability matters for a given sprint, record fleet-ids in that sprint's Running Context section by hand.
