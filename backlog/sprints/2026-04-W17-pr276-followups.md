...
- 2026-04-24 13:10 UTC (post-sprint follow-up): #284 closed via PR #293 (merged `40ab786`). Phase 1 rejected both Option A (new `relay-record-execution` command, ~200 LOC) and Option B (relax `recover-state --force`) in favor of a third path: **auto-rebrand `execution-evidence.json` inside `recover-commit.js` whenever it creates a new commit** (~35 prod LOC). Honesty check: recover-commit only runs `git add -A && git commit` on codex's leftover changes, so the new commit's tree is byte-identical to what codex tested — re-labeling is factually accurate, not a lie. Flow: codex dispatch (5.8 min, 176 LOC, completed-uncommitted) → recover-commit → r1 codex review returned `changes_requested` due to **anchor mismatch** (reviewer fetched issue body, didn't see the Phase 1 decision; `loadDoneCriteria` priority is `--done-criteria-file > manifest.anchor.done_criteria_path > github-issue-body > pr-body`). Workaround: spliced Phase 1 decision into issue #284 body via `gh issue edit` (NOT `gh issue comment` — comments are not part of body fetch), empty commit + `recover-state` advanced state to `review_pending`, r2 cleared the scope drift but found one legitimate gap (bad-SHA was throwing instead of returning structured `rejected_bad_sha` outcome — fixed in `443737c`, +20 LOC). Test baseline 822 → 842 (+20). Memory: `feedback_stale_execution_evidence_after_recovery` retired (workaround obsolete — auto-rebrand now handles it); `feedback_executor_did_not_open_pr` chained to mention auto-rebrand; new memory `feedback_phase1_decision_anchor_mismatch` added (workaround until #294 lands). Two new follow-up issues: **#294** (persist Phase 1 decisions into `manifest.anchor.done_criteria_path` so reviewer in fresh context sees operator's deviation) and **#295** (cleanup-worktrees rejects relay-owned worktrees as 'not relay-owned' — chicken-and-egg blocking 342 stale worktree pruning).
