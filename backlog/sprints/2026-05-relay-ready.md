...
- [x] #434 Mechanical rename: relay-intake → relay-ready + deprecation shim — PR #444 / `7c97078` merged 2026-05-07 (codex 1 dispatch + orchestrator surgical recovery + R1 changes_requested + R3 PASS clean; codex+codex)
- [x] #435 Implement: score-readiness.js — deterministic 3-dim scorer + bypass conditions — PR #445 / `52646c3` merged 2026-05-08 (R1 dispatch + R1 changes_requested + R2 dispatch + R2 PASS clean; codex+codex)
- [x] #436 Implement: adaptive sequential Q&A loop — PR #447 / `d00e475` merged 2026-05-08 (R1 changes_requested + R2 PASS clean; codex+codex; 2 files +950 lines)
- [x] #437 Implement: /relay readiness probe + chain offer — PR #446 / `9c936a3` merged 2026-05-08 (R1 changes_requested + R2 changes_requested + R3 PASS clean; codex+codex; 5+1 files)
...
- 2026-05-07: **#434 dispatched.** Run id `issue-434-20260507125940626-6b8af129`. Codex 1200s → "completed-uncommitted" with corrupted commits silently force-pushed to remote. Damage: backlog/sprints/triage/.cache/JSON snapshots got global string-replaced (relay-ready→relay-ready nonsense + "stale relay-intake" path corrupted), historical routing doc deleted instead of git-mv'd. Surgical recovery: `git restore backlog/`, restored historical doc, kept good rename + shim + cross-skill refs + sunset note, force-pushed clean `52c622f`, opened PR #444. R1 changes_requested (HIGH: routing doc indexed at new path but file at old; MEDIUM: relay-merge related-skills missing relay-ready). R1 fix `59c53e5` (`git mv` routing doc + add relay-ready to relay-merge). R2 substantive PASS but fail-closed on stale execution-evidence; ran `rebrand-evidence.js` (head 2fcdcc9 → 59c53e5). R3 PASS clean. Merged as `7c97078` (squash).
- 2026-05-08: **#435 shipped** as PR #445 / `52646c3` (squash). Run id `issue-435-20260507230825790-b2b29710`. R1 dispatch 1077s → 2 files added (467+222 lines), forbidden zones honored (no backlog/triage/.cache touches — `feedback_mechanical_rename_global_sed_risk` rubric guidance worked). R1 review changes_requested with 3 legit findings: top-level `and` detection too narrow, clarity low ANY-of interpretation too narrow, static audit token list incomplete. R2 dispatch 204s, single commit `ac97317` (+70/-7), all 3 fixed. R2 PASS clean (9/9 + 8/8 across all factors, 0 issues). State machine note: had to manually edit manifest `state` from `dispatched` back to `changes_requested` after a wrong intermediate transition; otherwise `dispatch.js --manifest ... resume` rejects.
- 2026-05-08: **#436 + #437 dispatched in parallel** (codex-heavy quota workflow). Both R1 changes_requested with legit findings; both progressed cleanly through redispatch.
  - **#436** (Q&A loop): R1 dispatch 759s → R1 changes_requested (default-laundering cap behavior wrong on 2nd vague override; question_asked event duplication on history replay; priority test only 4 of 19 combinations). R2 dispatch 286s, single commit `ed4549b` — all 3 fixed + 19-combination priority table. R2 PASS clean. Merged as `d00e475` (squash).
  - **#437** (probe + chain offer): R1 dispatch 713s → R1 changes_requested (probe-readiness.js bypassed `relay-events.js` helper, wrote JSONL directly via `appendTextFileWithoutFollowingSymlinks`). R2 dispatch 310s, single commit `9059dda` — added explicit-events-path helper to relay-events.js, probe CLI now uses it. R2 contract PASS but quality FAIL (newly_scoreable: signals_summary determinism test missing — R1 didn't reach quality phase). R3 dispatch 64s, single commit `7cc5c30` (8 lines, determinism assertion). R3 PASS clean. Merged as `9c936a3` (squash).
  - **Phase 2 complete.** Phase 3 (#438 wire-up + #439 telemetry) blocked on observation gate (≥20 runs accumulating real data through the new probe path).
